...
for (const record of names) {
  ...
  const structuredData = [
    { "@context": "https://schema.org", "@type": "Article", headline: `${record.name} name meaning`, description, mainEntityOfPage: canonical, about: { "@type": "DefinedTerm", name: record.name, description: record.meaning }, additionalProperty: phoneticProperty, isBasedOn: "Takiuddin Ahmed — Muslim Names Dataset (CC0 1.0)" },
  ...
  const staticBody = `<main class="static-name-page"><nav aria-label="Breadcrumb"><a href="/">Home</a> <span>/</span> <a href="/search">Names</a> <span>/</span> <span>${escape(record.name)}</span></nav><p class="kicker">${escape(record.gender)} name · Source-indexed record</p><h1>${escape(record.name)}</h1><p class="arabic" lang="ar" dir="rtl">${escape(record.arabic)}</p><p class="meaning">${escape(record.meaning)}</p><dl><div><dt>Arabic spelling</dt><dd>${escape(record.arabic)}</dd></div>${phoneticStatic}<div><dt>Gender</dt><dd>${escape(record.gender)}</dd></div><div><dt>First letter</dt><dd>${escape(record.letter)}</dd></div><div><dt>Source</dt><dd>Muslim Names Dataset (CC0 1.0)</dd></div></dl><section><h2>About ${escape(record.name)}</h2><p>${escape(record.description)}</p></section><p class="source-note">Meaning and Arabic spelling are sourced from the Muslim Names Dataset. Phonetic respellings are generated reading guides, not native-speaker recordings. See <a href="/sources">Sources &amp; methodology</a> for data limitations.</p></main>`;
