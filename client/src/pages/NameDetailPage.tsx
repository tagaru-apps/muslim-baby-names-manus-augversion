...
export default function NameDetailPage() {
  ...
  useEffect(() => {
    ...
    setPreviewUrl(null);
    createNameSocialImage(record, previewFormat, { includePhonetic }).then((blob) => {
      if (!active) return;
  ...
  }, [includePhonetic, previewFormat, previewOpen, record]);
    ...
    if (navigator.share) {
      try {
        ...
        if (navigator.canShare) {
          const image = await createNameSocialImage(record, "landscape", { includePhonetic });
  ...
  const downloadImage = async (format: SocialFormat = previewFormat) => {
    try {
      ...
      await downloadNameSocialImage(record, format, { includePhonetic });
  ...
  const shareStory = async () => {
    try {
      const image = await createNameSocialImage(record, "story", { includePhonetic });
