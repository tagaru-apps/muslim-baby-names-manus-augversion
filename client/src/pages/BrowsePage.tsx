/** Quiet Courtyard: editorial collection browsing with provenance-led filters and a compact archival reading list. */
...
export default function BrowsePage({ forcedGender, mode }: BrowseProps) {
  ...
  const routeOrigin = mode === "unique" ? queryParams.get("origin") || "all" : mode === "origin" ? routeParameter : "all";
  ...
  }, [location, mode, routeGender, routeOrigin]);
  ...
  const selectGender = (value: string) => { setGender(value); setVisibleCount(48); updateUniqueUrl(value, origin); };
  const selectOrigin = (value: string) => { setOrigin(value); setVisibleCount(48); updateUniqueUrl(gender, value); };
